...
{
 $query = "
  SELECT * FROM tb_pendaftaran ORDER BY no_pendaftaran LIMIT ".$_POST['p'].",5
 ";
}
 ...
 while($row = mysqli_fetch_array($result))
 {
  ...
  $output .= '
    <tr>
       ...
       <td><a href="?page=editpendaftaran&id='.$row["no_pendaftaran"].'"><button type="button" class="btn btn-info"><span class="glyphicon glyphicon-edit"></span> Edit</button></a><br><br>

        <button type="button" id='.$row["no_pendaftaran"].' onclick=konfirmasiHapus("pendaftaran",this.id) class="btn btn-warning"><span class="glyphicon glyphicon-remove"></span> Hapus</button></td>
       </tr>
  ';
 }
 $pagi = pagination($kon,'tb_pendaftaran',$_POST['p'],5,'datapendaftaran');
 $output.="</table>".$pagi;
 echo $output;
}
else
